...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        setImageList(fetchedProduct.galleryImage || []);
        setSelectedImage(fetchedProduct.galleryImage?.[0] || "");
      } catch (error) {
        console.error("Error fetching product:", error);
      }
    };
  ...
  if (!product) {
    return <div className="p-8 text-lg">Loading product...</div>;
  }
  ...
  return (
    <div className="product-main p-8">
      <div className="product-detail-container flex flex-col md:flex-row gap-8">
        <div className="product-image w-full md:w-1/2">
          ...
          <div className="product-box-row-img flex gap-4 mt-4 flex-wrap">
            {imageList.map((item, index) => (
              <div
                key={index}
                onClick={() => setSelectedImage(item)}
                className={`cursor-pointer p-1 border-2 rounded-xl ${
                  selectedImage === item ? "border-green-200" : "border-gray-300"
                }`}
              >
                <Image
                  src={item}
        ...
        <div className="product-detail-content w-full md:w-1/2">
          <div className="product-breadcrumbs text-gray-500">{productName}</div>
          <div className="product-title text-xl font-bold">{productName}</div>
...
const ProductDetailComp = () => {
  ...
  useEffect(() => {
    const fetchAllProducts = async () => {
      try {
        const response = await fetch(`https://api.purfull.com/product/get-all-product`);
  ...
  return (
    <div>
      ...
      <div className="font-semibold py-10 flex justify-between text-lg mx-18 mt-10">
        <span>More Items</span>
        <span className="text-[#2EAF4B] cursor-pointer">View All</span>
      </div>
