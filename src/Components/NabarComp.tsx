...
const NavbarComp = () => {
  ...
  const getAllproductapi = (value: string) => {
    const payload = {
      name: value,
    };
  ...
  const fetchSuggestions = async (text: string) => {
    try {
      const response = await fetch(
        `https://api.purfull.com/product/get-all-product?name=${encodeURIComponent(
          text
        )}`
      );
      if (!response.ok) {
    ...
    <>
      <div className="navbar">
        ...
        <div className="actions-tab-surface">
          ...
          <button className="cart-button" onClick={gocart}>
            <Image src="/Vectora.svg" alt="cart" width={24} height={24} />
            <span>cart</span>
          </button>
        </div>
        ...
        <AutoComplete
          className="search"
          options={suggestions}
          onSearch={(text: string) => setSearchterm(text.trim())}
          onSelect={(value) => router.push(`/productdetail/${value}`)}
        >
          <Input
            placeholder="Search by Tomato..."
            prefix={<SearchOutlined />}
        ...
        <div className="deleivery-and-carts">
          ...
          <div className="actions-mobile">
            ...
            {loggedInUser ? (
              ...
              <button className="login-button-mobile" onClick={goToLogin}>
                Login
              </button>
            )}
          </div>
