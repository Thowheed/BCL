...
// import { getUser } from "@/Globals/Localstorage";
import { getProductWiseIdLoad } from "@/store/reducer/indexSlice";
import { Button, message } from "antd";
import Image from "next/image";
import { useRouter } from "next/navigation";
import { useEffect, useState } from "react";
...
const CardComponent: React.FC<CardComponentProps> = ({ data }) => {
  ...
  const Cartapi = async (e: React.MouseEvent<HTMLButtonElement>) => {
    if (isadded) {
      router.push("/cart");
      return;
    }
  ...
  }, [productLoad, data?.id, router]);

  // Parse name and description by current language
  const lang = i18n.language;
